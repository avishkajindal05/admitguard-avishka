  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-[#1F2937] dark:text-[#F1F5F9]">Dashboard Overview</h1>
        <div className="text-sm text-[#9CA3AF] dark:text-[#64748B] font-medium">
          ...
          return (
            <Card key={i} className="hover:shadow-md transition-shadow">
              <div className="flex items-start justify-between">
                <div>
                  <p className="text-xs font-bold text-[#9CA3AF] dark:text-[#64748B] uppercase tracking-wider mb-2">{stat.label}</p>
                  <p className="text-3xl font-bold text-[#1F2937] dark:text-[#F1F5F9]">{stat.value}</p>
                </div>
          ...
          <div className="h-72 w-full pt-4">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#EAECEF" className="dark:stroke-[#334155]" />
                <XAxis 
                  dataKey="name" 
                  axisLine={false} 
                  tickLine={false} 
                ...
                />
                <Tooltip 
                  cursor={{ fill: '#F8F9FB', opacity: 0.1 }}
                  contentStyle={{ 
                    borderRadius: '12px', 
                    border: 'none', 
              ...
              <div key={i} className="space-y-2">
                <div className="flex justify-between items-end">
                  <div className="space-y-0.5">
                    <p className="text-[10px] font-bold text-[#9CA3AF] dark:text-[#64748B] uppercase tracking-wider">{item.label}</p>
                    <p className="text-sm font-bold text-[#1F2937] dark:text-[#F1F5F9]">{item.count} <span className="text-[#9CA3AF] dark:text-[#64748B] font-medium">Entries</span></p>
                  </div>
                  <span className="text-xs font-bold text-primary">{item.percentage}%</span>
                </div>
                <div className="h-2 bg-[#F3F4F6] dark:bg-[#0F172A] rounded-full overflow-hidden">
                  <div 
