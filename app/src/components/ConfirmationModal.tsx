    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl overflow-hidden animate-in zoom-in-95 duration-300">
        <div className="px-6 py-4 border-b border-[#EAECEF] flex items-center justify-between bg-[#F8F9FB]">
          ...
          <button onClick={onClose} className="p-1 hover:bg-[#EAECEF] rounded-full transition-colors">
        ...
        <div className="p-6 max-h-[70vh] overflow-y-auto space-y-6">
            ...
            {Object.entries(FIELD_CONFIG).map(([key, config]) => {
              ...
              return (
                <div key={key} className="space-y-1">
                  ...
                  <p className="text-sm font-medium text-[#1F2937]">{displayVal}</p>
                </div>
              );
            })}
          </div>
          ...
          {exceptions.length > 0 && (
            <div className="space-y-3 pt-4 border-t border-[#EAECEF]">
              ...
              <div className="space-y-3">
                {exceptions.map((ex, idx) => (
                  <div key={idx} className="bg-soft-bg/20 border border-soft-border/30 p-3 rounded-lg">
        ...
        <div className="px-6 py-4 bg-[#F8F9FB] border-t border-[#EAECEF] flex items-center justify-end gap-3">
          <button
            onClick={onClose}
            className="px-6 h-11 rounded-lg border border-[#D1D5DB] text-sm font-semibold text-[#4B5563] hover:bg-white transition-all"
          >
            Cancel
          </button>
          <button
